...
class FollowersParser(object):
    def __init__(self):
        ...
        self.appid = 552990
        self.followers_url = f'http://steamcommunity.com/games/{self.appid}/memberslistxml/?xml=1&l=english'+'&p={}'
        self.use_proxy_max = 10
        self.use_proxy = 0
        self.total_pages = 0
    ...
    def get_steamids_from_page(self, page_number):
        ...
        while repeat is True:
            res = requests.get(self.followers_url.format(page_number), proxies=self.proxy.active_proxy, timeout=5)
            self.use_proxy += 1
            if res.status_code == 200 and self.use_proxy <= self.use_proxy_max:
                soup = BeautifulSoup(res.text, 'xml')
    ...
    def get_steamids_all_pages(self):
        ...
        for page_number in range(1, self.total_pages+1):
            self.get_steamids_from_page(page_number)
